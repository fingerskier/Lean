(function () {
    ...
    function Wb(a, b) {
        if ("array" == ea(a))for (var c = 0; c < a.length; ++c)b(c, a[c]); else Xb(a, b)
    }
    ...
    yc.prototype.open = function (a, b) {
        ...
        this.Y.onmessage = function (a) {
            if (c.Y !== l)if (a = a.data, rc(c.$, "bytes_received", a.length), Ac(c), c.frames !== l)Bc(c, a); else {
                a:{
    ...
    function Qc(a, b) {
        return function (c) {
            if (2 != a.va)if (b === a.Ib) {
                ...
                if ("c" == d) {
                    if (d = Ub("t", c), "d"in c)if (c = c.d, "h" === d) {
                        var d = c.ts, e = c.v, f = c.h;
                        a.gc = c.s;
                        rb(a.K, f);
                        if (0 == a.va && (a.L.start(), c = a.L, a.e("Realtime connection established."), a.L = c, a.va = 1, a.yb && (a.yb(d), a.yb = l), "5" !== e && P("Protocol version mismatch detected"), c = 1 < a.Tc.ic.length ? a.Tc.ic[1] : l))a.A = new c("c:" + a.id + ":" + a.Vc++, a.K, a.gc), a.A.open(Qc(a, a.A), Rc(a, a.A))
                    } else if ("n" === d) {
                        ...
                        a.Yb = [];
                        Sc(a)
                    } else"s" === d ? (a.e("Connection shutdown command received. Shutting down..."), a.Fc && (a.Fc(c), a.Fc = l), a.R = l, a.close()) : "r" === d ? (a.e("Reset packet received.  New host: " + c), rb(a.K, c), 1 === a.va ? a.close() : (Tc(a), Pc(a))) : "e" === d ? Pb("Server Error: " + c) : Pb("Unknown control packet command: " + d)
                } else"d" == d && a.Vb(c)
            } else b === a.A ? (d = Ub("t", c), c = Ub("d", c), "c" == d ? "t"in c && (c = c.t, "a" === c ? (a.A.start(), a.e("sending client ack on secondary"), a.A.send({t: "c", d: {t: "a",
    ...
    function ud(a, b, c, d, e) {
        ...
        for (var h = 0; h < f.length; h++)if (f[h].type === b) {
            ...
            f[h].aa && (i = i.F(f[h].aa));
            d.push({W: e ? w(c, e) : c, md: new T(f[h].sa, i), Eb: f[h].Eb})
        }
        rd(a.uc, d)
    }
    ...
    function Dd(a, b, c, d, e) {
        ...
        O = d.Va();
        for (G = Wa(O); I !== l || G !== l;) {
            c = I === l ? 1 : G === l ? -1 : I.key === G.key ? 0 : cc({name: I.key, ha: I.value.j()}, {name: G.key, ha: G.value.j()});
        ...
        for (h = 0; h < m.length; h++)if (q = m[h])c = b.F(q.key), Cd(a, c, q.value, Q), e.push(new W("child_removed", q.value, q.key));
        for (h = 0; h < i.length; h++)if (q = i[h])c = b.F(q.key), m = d.da(q.key, q.value), Cd(a, c, Q, q.value), e.push(new W("child_added", q.value, q.key, m));
    ...
    s.update = function (a, b, c) {
        ...
        if (e)N("update() called with empty data.  Don't do anything."), X(c, "ok"); else {
            var q = this;
    ...
    function ae(a, b) {
        ...
        if (0 !== e.length) {
            for (var f = c = U(a.g.la, d), h = [], i = 0; i < e.length; i++) {
                ...
                n && (e[i].status =
                    3, setTimeout(e[i].kc, 0), e[i].C && (n = new J(a, e[i].path), m = new T(c.Q(m), n), "nodata" === p ? h.push(w(e[i].C, l, l, o, m)) : h.push(w(e[i].C, l, Error(p), o, m))))
            }
            p = U(a.g.I, d).j();
            c = c.Ea(p);
            f = f.Ea(p);
    ...
    $.prototype.update = function (a, b) {
        ...
        for (f in a)e = o;
        if (e)N("onDisconnect().update() called with empty data.  Don't do anything."), X(b, k); else {
            e = c.u;
            f = d.toString();
            var h = function (e) {
